(function () {
  ...
  function notifyByEmail(payload) {
    ...
    if (typeof sendNotificationEmail === 'function') {
      Promise.resolve()
        ...
        .catch((error) => console.error('❌ فشل إرسال إشعار البريد للبرامج الداخلية - غير الموظفين:', error));
    }
  }
  ...
  async function persistInternalOthers(formData) {
    const cleaned = {};
    ...
    cleaned.id = Date.now();
    cleaned.formType = 'internal-others';
    cleaned.status = 'Pending';
    cleaned.assignedTo = 'الموظف الإداري';
    ...
    if (window.db) {
      try {
        ...
        saveLocal('internal-others', persistable);
  ...
  async function handleInternalOthersSubmit(event) {
    ...
    try {
      ...
      const result = await persistInternalOthers(payload);
      if (!result.success) {
      ...
      const previewData = typeof window.normalizePreviewData === 'function'
        ? window.normalizePreviewData('internal-others', result.data)
        : result.data;
      window.__lastSubmissionPreviewData = previewData;

      if (typeof window.showSuccessMessage === 'function') {
        window.showSuccessMessage('internal-others', previewData, !!result.local);
      }
      if (typeof window.openSubmissionPrintPreview === 'function') {
        try {
          window.openSubmissionPrintPreview('internal-others', previewData);
        } catch (_) {}
      }
  ...
  window.addEventListener('DOMContentLoaded', function () {
    const originalForm = document.getElementById('internal-others-form');
    ...
    replacementForm.addEventListener('submit', handleInternalOthersSubmit);
  });
})();